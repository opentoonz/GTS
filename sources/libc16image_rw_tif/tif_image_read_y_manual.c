...
static void *_tif_image_read_y_manual( TIF_IMAGE_RW *tp_read, long *lp_width, long *lp_height, long l_h_read, long *lp_channels, long *lp_bytes, long *lp_bits, double *dp_dpi_x, double *dp_dpi_y, long *lp_tile_width, long *lp_tile_height, long *lp_how_to_compression, long *lp_orientation, int i_mv_sw, int i_pv_sw, int i_cv_sw )
{
	int	i_reverse_sw;
	void	*vp_image;
...
int i_cv_sw )
{
	...
	/* ファイルを読む */
	vp_image = _tif_image_read_y_manual(
		&t_read,
		lp_width,
		lp_height,
		l_h_read,
	...
	if (NULL == vp_image) {
		pri_funct_err_bttvr(
		"Error : _tif_image_read_y_manual(-) returns NG." );
	}
