#include "memory_config.h"
#include "gts_gui.h"
#include "gts_master.h"

//--------------------------------------------------
void memory_config::save_trace_src_hsv_( std::ofstream& ofs )
{
	unsigned char uchar_red, uchar_gre, uchar_blu;

	/* 01 ------------------------------------------------ */
	this->save_word_( this->str_color_trace_01_chk_
		, cl_gts_gui.chkbtn_color_trace_01_chk->value()
		  ? this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_01_src_hh_min_
		, cl_gts_gui.valinp_color_trace_01_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_01_src_hh_max_
		, cl_gts_gui.valinp_color_trace_01_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_01_src_aa_min_
		, cl_gts_gui.valinp_color_trace_01_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_01_src_aa_max_
		, cl_gts_gui.valinp_color_trace_01_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_01_src_bb_min_
		, cl_gts_gui.valinp_color_trace_01_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_01_src_bb_max_
		, cl_gts_gui.valinp_color_trace_01_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_01 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_01_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_01_tgt_color_
		,   cl_gts_gui.roubut_thickness_01_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/* 02 ------------------------------------------------*/
	this->save_word_( this->str_color_trace_02_chk_
		, cl_gts_gui.chkbtn_color_trace_02_chk->value()
		  ?  this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_02_src_hh_min_
		, cl_gts_gui.valinp_color_trace_02_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_02_src_hh_max_
		, cl_gts_gui.valinp_color_trace_02_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_02_src_aa_min_
		, cl_gts_gui.valinp_color_trace_02_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_02_src_aa_max_
		, cl_gts_gui.valinp_color_trace_02_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_02_src_bb_min_
		, cl_gts_gui.valinp_color_trace_02_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_02_src_bb_max_
		, cl_gts_gui.valinp_color_trace_02_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_02 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_02_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_02_tgt_color_
		,   cl_gts_gui.roubut_thickness_02_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/* 03 ------------------------------------------------*/
	this->save_word_( this->str_color_trace_03_chk_
		, cl_gts_gui.chkbtn_color_trace_03_chk->value()
		  ?  this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_03_src_hh_min_
		, cl_gts_gui.valinp_color_trace_03_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_03_src_hh_max_
		, cl_gts_gui.valinp_color_trace_03_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_03_src_aa_min_
		, cl_gts_gui.valinp_color_trace_03_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_03_src_aa_max_
		, cl_gts_gui.valinp_color_trace_03_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_03_src_bb_min_
		, cl_gts_gui.valinp_color_trace_03_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_03_src_bb_max_
		, cl_gts_gui.valinp_color_trace_03_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_03 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_03_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_03_tgt_color_
		,   cl_gts_gui.roubut_thickness_03_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/* 04 ------------------------------------------------*/
	this->save_word_( this->str_color_trace_04_chk_
		, cl_gts_gui.chkbtn_color_trace_04_chk->value()
		  ?  this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_04_src_hh_min_
		, cl_gts_gui.valinp_color_trace_04_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_04_src_hh_max_
		, cl_gts_gui.valinp_color_trace_04_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_04_src_aa_min_
		, cl_gts_gui.valinp_color_trace_04_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_04_src_aa_max_
		, cl_gts_gui.valinp_color_trace_04_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_04_src_bb_min_
		, cl_gts_gui.valinp_color_trace_04_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_04_src_bb_max_
		, cl_gts_gui.valinp_color_trace_04_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_04 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_04_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_04_tgt_color_
		,   cl_gts_gui.roubut_thickness_04_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/* 05 ------------------------------------------------*/
	this->save_word_( this->str_color_trace_05_chk_
		, cl_gts_gui.chkbtn_color_trace_05_chk->value()
		  ?  this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_05_src_hh_min_
		, cl_gts_gui.valinp_color_trace_05_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_05_src_hh_max_
		, cl_gts_gui.valinp_color_trace_05_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_05_src_aa_min_
		, cl_gts_gui.valinp_color_trace_05_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_05_src_aa_max_
		, cl_gts_gui.valinp_color_trace_05_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_05_src_bb_min_
		, cl_gts_gui.valinp_color_trace_05_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_05_src_bb_max_
		, cl_gts_gui.valinp_color_trace_05_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_05 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_05_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_05_tgt_color_
		,   cl_gts_gui.roubut_thickness_05_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/* 06 ------------------------------------------------*/
	this->save_word_( this->str_color_trace_06_chk_
		, cl_gts_gui.chkbtn_color_trace_06_chk->value()
		  ?  this->str_on_:this->str_off_ ,ofs );
	this->save_f64_(  this->str_color_trace_06_src_hh_min_
		, cl_gts_gui.valinp_color_trace_06_src_hh_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_06_src_hh_max_
		, cl_gts_gui.valinp_color_trace_06_src_hh_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_06_src_aa_min_
		, cl_gts_gui.valinp_color_trace_06_src_aa_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_06_src_aa_max_
		, cl_gts_gui.valinp_color_trace_06_src_aa_max->value(),ofs);
	this->save_f64_(  this->str_color_trace_06_src_bb_min_
		, cl_gts_gui.valinp_color_trace_06_src_bb_min->value(),ofs);
	this->save_f64_(  this->str_color_trace_06_src_bb_max_
		, cl_gts_gui.valinp_color_trace_06_src_bb_max->value(),ofs);
	cl_gts_master.cl_color_trace_enhancement.tgt_get_uchar_rgb_color(
		E_COLOR_TRACE_HAB_06 , &uchar_red, &uchar_gre, &uchar_blu );
	this->save_3int_( this->str_color_trace_06_tgt_rgb_
		, uchar_red, uchar_gre, uchar_blu ,ofs );
	this->save_str_(  this->str_color_trace_06_tgt_color_
		,   cl_gts_gui.roubut_thickness_06_tgt_is_bl->value() == 1
		  ? this->str_color_black_ : this->str_color_not_black_
		, ofs
	);
	/*------------------------------------------------*/
}

