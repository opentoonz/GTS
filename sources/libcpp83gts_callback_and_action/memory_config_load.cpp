...
)
{
	...
	for (int ii = 1 ;std::getline(ifs,str) ;++ii) {/* 一行読む */
		...
		if (this->load_scan_and_save_( words
		 ,scan_num_continue_type_sw)) { continue; }
		...
		if (this->load_trace_batch_(words
		 ,load_trace_batch_sw ,trace_batch_list_sw)) { continue; }
		if (this->load_number_(words
		 ,number_list_sw)) { continue; }
...
)
{
	...
	) {    ss.cb_switch_scan_filter_erase_dot_noise(va==this->str_on);
	}

	else if ( (this->str_scan_save_dir_path_ == ke)
	||	  (this->str_scan_save_dir_path_legacy2017_ == ke)
	...
	||	  (this->str_scan_save_file_head_legacy2017_ == ke)
	...
	||	  (this->str_scan_save_image_format_legacy2017_ == ke)
...
bool memory_config::load_trace_files_( std::vector< std::string >& words )
{
	...
	else if (  this->str_trace_open_number_format == ke ) {
	   cl_gts_gui.strinp_trace_open_number_format->value( va.c_str() );
	}
	...
	else if (  this->str_trace_num_start == ke ) {
	   cl_gts_gui.strinp_trace_num_start->value( va.c_str() );
	}
	else if (  this->str_trace_num_end == ke ) {
	   cl_gts_gui.strinp_trace_num_end->value( va.c_str() );
	}
	/* trace_swはON固定 */
	else if ( this->str_trace_filter_erase_dot_noise_sw_ == ke
	) {    ss.cb_switch_trace_filter_erase_dot_noise(va==this->str_on);
	}
	else if (  this->str_trace_save_dir_path == ke ) {
	   cl_gts_gui.filinp_trace_save_dir_path->value( va.c_str() );
	}
	else if (  this->str_trace_save_file_head == ke ) {
	   cl_gts_gui.filinp_trace_save_file_head->value( va.c_str() );
	}
	else if (  this->str_trace_save_number_format == ke ) {
	   cl_gts_gui.filinp_trace_save_number_format->value( va.c_str() );
	}
	else if (  this->str_trace_save_image_format == ke ) {
...
((Fl_Valuator *)cl_gts_gui.scrbar_bw_threshold)->value( std::stod(va) );
		...
		cl_gts_gui.valinp_grays_brightness->value( std::stod(va) );
((Fl_Valuator *)cl_gts_gui.scrbar_grays_brightness)->value( std::stod(va) );
	}
	else if ( ke == this->str_grayscale_contrast_ ) {
		cl_gts_gui.valinp_grays_contrast->value( std::stod(va) );
((Fl_Valuator *)cl_gts_gui.scrbar_grays_contrast)->value( std::stod(va) );
	}
	else if ( ke == this->str_grayscale_gamma_ ) {
		cl_gts_gui.valinp_grays_gamma->value( std::stod(va) );
((Fl_Valuator *)cl_gts_gui.scrbar_grays_gamma)->value( std::stod(va) );
	}
...
)
{
	...
	else if (this->str_trace_batch_dir_ == ke) {
