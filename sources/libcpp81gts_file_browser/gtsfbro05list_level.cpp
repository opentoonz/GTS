...
int gtsfbro05list_level::change_level_list( void )
{
	...
	ccp_gui_main_dir= cl_gts_gui.filinp_level_save_dir->value();
	ccp_gui_sub_dir = cl_gts_gui.filinp_level_open_dir->value();
...
void gtsfbro05list_level::change_level_dir( const char *ccp_dir )
{
	...
	if (0 == cl_gts_gui.togbut_level_open_browse->value()) {
	 cl_gts_gui.filinp_level_save_dir->value(cp_gui_dir);
	 cl_gts_gui.filinp_level_save_dir->position(strlen(cp_gui_dir));
	}
	cl_gts_gui.filinp_level_open_dir->value(cp_gui_dir);
...
{
	cl_gts_gui.filinp_level_save_dir->value(ccp_dir);
	cl_gts_gui.filinp_level_save_dir->position(strlen(ccp_dir));
