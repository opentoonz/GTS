...
void gtsfbro06cb_level::memory_from_gui( const char* level_dir_path )
{
	...
	// save file head
	this->save_file_head_ = cl_gts_gui.strinp_level_file->value();

	// save and open start
	this->saveopen_start_ = cl_gts_gui.valinp_level_start->value();

	// save and open end
	this->saveopen_end_   = cl_gts_gui.valinp_level_end->value();
...
{
	// save dir path
	cl_gts_gui.filinp_level_save_dir->value( this->cp_memory() );
	cl_gts_gui.filinp_level_save_dir->position(
		strlen(this->cp_memory())
	);

	// save file head
	cl_gts_gui.strinp_level_file->value( this->save_file_head_.c_str());

	// save and open start
	cl_gts_gui.valinp_level_start->value(this->saveopen_start_ );

	// save and open end
	cl_gts_gui.valinp_level_end->value(  this->saveopen_end_ );
	...
	if (cl_gts_gui.choice_level_continue_type->text()
	== std::string("End")) {
	...
	if (!(this->save_image_format_.empty())) {
	 cl_gts_gui.choice_level_image_file_format->value(
	  cl_gts_gui.choice_level_image_file_format->find_item(
